...
#            ct_id_stat = st.selectbox("Cottage Status ID", options=[cs['cottage_status_id'] for cs in cottage_statuses], index=cottage_statuses.index(next(filter(lambda x: x['cottage_status_id'] == current_attributes['ct_id_stat'], cottage_statuses))) if cottage_statuses else None)
            ...
            pool_id = st.selectbox("Pool", options=[f"{pool['pool_id']} : {pool['pool_detail']}" for pool in pools], index=pools.index(next(filter(lambda x: x['pool_id'] == current_attributes['pool_id'], pools))) if pools else None)
            loc_id = st.selectbox("Location", options=[f"{location['loc_id']} : {location['loc_details']}" for location in locations], index=locations.index(next(filter(lambda x: x['loc_id'] == current_attributes['loc_id'], locations))) if locations else None)
            room_id = st.selectbox("Room", options=[f"{room['room_id']} : {room['room_details']}" for room in rooms], index=rooms.index(next(filter(lambda x: x['room_id'] == current_attributes['room_id'], rooms))) if rooms else None)
            max_pax_id = st.selectbox("Max Pax", options=[f"{max_pax['max_pax_id']} : {max_pax['max_pax_details']}" for max_pax in maximum_pax], index=maximum_pax.index(next(filter(lambda x: x['max_pax_id'] == current_attributes['max_pax_id'], maximum_pax))) if maximum_pax else None)
            ct_id = st.selectbox("Cottage Type", options=[f"{ct['ct_id']} : {ct['ct_details']}" for ct in cottage_types], index=cottage_types.index(next(filter(lambda x: x['ct_id'] == current_attributes['ct_id'], cottage_types))) if cottage_types else None)
            ct_id_stat = st.selectbox("Cottage Status", options=[f"{cs['cottage_status_id']} : {cs['ct_status_details']}" for cs in cottage_statuses], index=cottage_statuses.index(next(filter(lambda x: x['cottage_status_id'] == current_attributes['cottage_status_id'], cottage_statuses))) if cottage_statuses else None)    
